...
if (isset($_POST["report"])){
	...
	if ($_POST["report"]=="detail"){
		...
		$getTable->close();
		$conn->close();
		$file="détaillé".date("Y-m-d h:i:sa").".xls";
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment; filename=$file");
	}
	else if($_POST["report"]=="summary"){
		...
		{
			echo "<p style='text-align:center;color:#e7d732'>totalStudents Il n'y a pas de rapport.</p>";
		}
		...
		$conn->close();
		$file="sommaire".date("Y-m-d h:i:sa").".xls";
		header("Content-type: application/vnd.ms-excel");
		header("Content-Disposition: attachment; filename=$file");
	}
	else if($_POST["report"]=="exception"){
