<?php
include("headerLayout.php");
?>

<div class="row">
	<h2>Mes Techniques</h2>
</div>
<div id="table">
<table class="tg">
	<tr>
		<th class="tg-amwm">Techniques</th>
		<th class="tg-amwm"><font color="white">Blanche</font></th>
		<th class="tg-amwm"><font color="#ddd840">Jaune</font></th>
		<th class="tg-amwm"><font color="orange">Orange</font></th>
		<th class="tg-amwm"><font color="#d365cf">Violette</font></th>
		<th class="tg-amwm"><font color="#4286f4">Bleue</font></th>
		<th class="tg-amwm"><font color="green">Verte</font></th>
		<th class="tg-amwm"><font color="#93503c">Brune</font></th>
		<th class="tg-amwm"><font color="black">Noire</font></th>
	</tr>
	<tr>
		<td class="tg-amwm">Coups de Poing</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">7</td>
	</tr>
	<tr>
		<td class="tg-amwm">Coups de Pied</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">5</td>
	</tr>
	<tr>
		<td class="tg-amwm">Bloques</td>
		<td class="tg-baqh">8</td>
		<td class="tg-baqh">8</td>
		<td class="tg-baqh">8</td>
		<td class="tg-baqh">3</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">3</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
	</tr>
	<tr>
		<td class="tg-amwm">Coups de Genou</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">1</td>
		<td class="tg-baqh">1</td>
		<td class="tg-baqh">1</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">1</td>
	</tr>
	<tr>
		<td class="tg-amwm">Coups d'Épaule</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">7</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">0</td>
	</tr>
	<tr>
		<td class="tg-amwm">Combinaisons</td>
		<td class="tg-baqh">3,6,7</td>
		<td class="tg-baqh">12,18</td>
		<td class="tg-baqh">2,3</td>
		<td class="tg-baqh">8,9</td>
		<td class="tg-baqh">10,15,16</td>
		<td class="tg-baqh">11,13,14</td>
		<td class="tg-baqh">1,21,19,26</td>
		<td class="tg-baqh">22</td>
	</tr>
	<tr>
		<td class="tg-amwm">Kempos</td>
		<td class="tg-baqh">0</td>
		<td class="tg-baqh">3</td>
		<td class="tg-baqh">3</td>
		<td class="tg-baqh">4</td>
		<td class="tg-baqh">6</td>
		<td class="tg-baqh">10</td>
		<td class="tg-baqh">12</td>
		<td class="tg-baqh">5</td>
	</tr>
	<tr>
		<td class="tg-amwm">Formes</td>
		<td class="tg-baqh">Pinan 1</td>
		<td class="tg-baqh">Pinan 2</td>
		<td class="tg-baqh">Kata 1</td>
		<td class="tg-baqh">Kata 2</td>
		<td class="tg-baqh">Pinan 3, Pinan 4, Forme de Bloques</td>
		<td class="tg-baqh">Pinan 5, Forme de la Grue</td>
		<td class="tg-baqh">Kata 3, Kata 4, Kata 5, Forme du Prunier, Two Man Fist Set A</td>
		<td class="tg-baqh">Kata 6, Ansuki, Two Man Fist Set B</td>
	</tr>
</table>
</div>

<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_white">
    Blanche <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
    <li>Bloques: 8</li>
	<li>Combinaisons: 3,6,7</li>
	<li>Formes: Pinan 1</li>
  </ul>
</div>

<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_yellow">
    Jaune <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
    <li>Bloques: 8</li>
    <li>Coups de Genou: 1</li>
	<li>Coups d'Épaule: 7</li>
	<li>Combinaisons: 12,18</li>
	<li>Kempos: 3</li>
	<li>Formes: Pinan 2</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_orange">
    Orange <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
    <li>Bloques: 8</li>
    <li>Coups de Genou: 1</li>
	<li>Combinaisons: 2,3</li>
	<li>Kempos: 3</li>
	<li>Formes: Kata 1</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_purple">
    Violette <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
    <li>Bloques: 3</li>
    <li>Coups de Genou: 1</li>
	<li>Combinaisons: 8,9</li>
	<li>Kempos: 4</li>
	<li>Formes: Kata 2</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_blue">
    Bleue <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
	<li>Combinaisons: 10,15,16</li>
	<li>Kempos: 6</li>
	<li>Formes: Pinan 3, Pinan 4, Forme de Bloques</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_green">
    Verte <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
    <li>Bloques: 3</li>
	<li>Combinaisons: 11,13,14</li>
	<li>Kempos: 10</li>
	<li>Formes: Pinan 5, Forme de la Grue</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_brown">
    Brune <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 4</li>
	<li>Combinaisons: 1,21,19,26</li>
	<li>Kempos: 12</li>
	<li>Formes: Kata 3, Kata 4, Kata 5, Forme du Prunier, Two Man Fist Set A</li>
  </ul>
</div>
<div class="btn-group">
  <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" id="btn_black">
    Noire <span class="caret"></span>
  </button>
  <ul class="dropdown-menu">
    <li>Coups de Poing: 7</li>
    <li>Coups de Pied: 5</li>
    <li>Coups de Genou: 1</li>
	<li>Combinaisons: 22</li>
	<li>Kempos: 5</li>
	<li>Formes: Kata 6, Ansuki, Two Man Fist Set B</li>
  </ul>
</div>
<?php
include("footerLayout.php");
?>